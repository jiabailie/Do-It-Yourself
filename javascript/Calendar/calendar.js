...
.constant('DEFAULT_YEAR_PREFIX', '20')
.service('turnCalendarService', ['MAX_MONTHS_ALLOWED', 'MIN_MONTHS_ALLOWED', 'DEFAULT_YEAR_PREFIX', function (MAX_MONTHS_ALLOWED, MIN_MONTHS_ALLOWED, DEFAULT_YEAR_PREFIX) {
    return {
        ...
        appendZeroesLeft: function (size, strData) {
            if (!/^\d+$/.test(strData)) {
                return strData;
            }
            strData = '' + strData;
            while (strData.length < size) {
                strData = '0' + strData;
            }
            return strData;
        },
        handleInputDateStringFormat: function (strDate, dateFormat) {
            var yearPattern = '';
            var unifiedDateFormat = [];
            strDate = strDate.replace(/[-/.,]/g, " ").split(" ");
            dateFormat = dateFormat.toLowerCase().replace(/[-/.,]/g, " ").split(" ");
            for (var i = 0; i < dateFormat.length; ++i) {
                if (dateFormat[i].length > strDate[i].length) {
                    strDate[i] = this.appendZeroesLeft(dateFormat[i].length, strDate[i]);
                }
                if (dateFormat[i][0] == 'y') {
                    if (dateFormat[i].length == 2) { strDate[i] = DEFAULT_YEAR_PREFIX + strDate[i]; }
                    unifiedDateFormat[2] = strDate[i];
                } else if (dateFormat[i][0] == 'd') {
                    unifiedDateFormat[1] = strDate[i];
                } else if (dateFormat[i][0] == 'm') {
                    unifiedDateFormat[0] = strDate[i];
                }
            }
            return unifiedDateFormat[0] + "/" + unifiedDateFormat[1] + "/" + unifiedDateFormat[2];
        },
        convertToDateU: function (date, dateFormat) {
            if (!date) {
                return false;
            }
            date = this.handleInputDateStringFormat(date, dateFormat);
            var dateObj = new Date(Date.parse(date));
            if (Object.prototype.toString.call(dateObj) !== '[object Date]')
                return new Date();
            return dateObj;
        },
        validateDateInput: function (date, dateFormat) {
            if (!date) {
                return false;
            }
            try {
                ...
                // Encounter the error when convert input to date.
                // It means that input is an invalid date time.
                return false;
            }
        },
...
.controller('CalendarController', [
  ...
  'turnCalendarService',
  '$document',
  function ($rootScope, $scope, $attrs, turnCalendarDefaults, turnCalendarService, $document) {
      var self = this;
      var calendarOptions;
      var MONTH_NAME;
      ...
      /**
        ...
        * @type {boolean}
        */
      var allowMonthGeneration = false;
      ...
      var pickValue = function (property) {
          if (angular.isDefined($attrs[property])) {
              if ($scope.$parent.$parent.$eval($attrs[property]) != null)
                  return $scope.$parent.$parent.$eval($attrs[property]);
              else if ($scope.$parent.$eval($attrs[property]) != null)
      ...
      angular.forEach([
        ...
        'bindingIndex'
          ...
          self[key] = $scope[key] = pickValue(key);
      });
      $scope.realBindingIndex = $scope.bindingIndex;
      angular.forEach([
        'minSelectDate',
        'maxSelectDate'
      ], function (key) {
          $scope.$watch(key, function (newVal) {
              if (!turnCalendarService.validateDateInput(newVal, self.dateFormat)) {
                  return;
              }
              newVal = turnCalendarService.convertToDate(newVal, self.dateFormat);
              self[key] = newVal;
              ...
              var endDate = resetEndDate(selectedEndDate.date);
      ...
      $scope.dateFormat = self.dateFormat;
      $scope.isNotSingleDateMode = self.selectionMode !== 'singleDate';

      $scope.getInputStartDate = function () {
          var strDateFormat = self.dateFormat;
          var strStartDate = $scope.selectionDateString;
          self.startDate = $scope.startDate = strStartDate;
          self.endDate = $scope.endDate = strStartDate;
          if ($scope.isNotSingleDateMode) {
              strStartDate = $scope.selectionDateString.substring(0, strDateFormat.length);
          }
          if (strStartDate == null || !turnCalendarService.validateDateInput(strStartDate, strDateFormat)) {
              return new Date();
          }
          return turnCalendarService.convertToDateU(strStartDate, strDateFormat);
      };
      $scope.getInputEndDate = function () {
          if (!$scope.isNotSingleDateMode) {
              return $scope.getInputStartDate();
          }
          var strDateFormat = $scope.dateFormat;
          var strEndDate = $scope.selectionDateString;
          if (strEndDate == null) { return new Date(); }
          var iEndDatePos = strEndDate.length - strDateFormat.length;
          strEndDate = strEndDate.substring(iEndDatePos);
          self.endDate = $scope.endDate = strEndDate;
          if (strEndDate == null || !turnCalendarService.validateDateInput(strEndDate, strDateFormat)) {
              return new Date();
          }
          return turnCalendarService.convertToDateU(strEndDate, strDateFormat);
      }
      /**
      ...
      $scope.dayNames = [];
      /**
           * An integer which is the year of current selected day, to be displayed
           * by template
           *
           * @type {array}
           */
      ...
      var dayRemained = tempDayName.splice(self.startDayOfWeek);
      ...
      var resetStartDate = function (startDate) {
          return resetDateValid(startDate);
      };
      var resetDateValid = function (objDate) {
          var minDate = self.minSelectDate ? new Date(self.minSelectDate) : new Date("1/1/0001");
          var maxDate = self.maxSelectDate ? new Date(self.maxSelectDate) : new Date("1/1/9999");
          if (objDate.getTime() < minDate.getTime()) { objDate = minDate; }
          if (objDate.getTime() > maxDate.getTime()) { objDate = maxDate; }
          return objDate;
      }      /**
      ...
      var generateMonthArray = function (inputYear, inputMonth) {
          var startDate = resetStartDate($scope.getInputStartDate());
      ...
      var getFirstDate = function (year, month) {
          ...
          var firstDate = new Date(firstDayOfMonth.setDate(generateFirstDateIndex(self.startDayOfWeek, dayOfWeek)));
          return firstDate;
      };
      /**
      ...
      var isUnavailable = function (date) {
          return self.minSelectDate &&
              date < new Date(self.minSelectDate) ||
              self.maxSelectDate &&
              date > new Date(self.maxSelectDate);
      };
      /**
      ...
      var isDaily = function () {
          return !self.weeklySelectRange &&
              !self.monthlySelectRange ||
      ...
      var isBetweenStartAndEndDate = function (date) {
          ...
          return date.setHours(0, 0, 0, 0) <= selectedEndDate.date.setHours(0, 0, 0, 0) &&
              date.setHours(0, 0, 0, 0) >= selectedStartDate.date.setHours(0, 0, 0, 0);
      };
      /**
      ...
      var colorSelectedDateRange = function () {          
          var objDate = $scope.getInputStartDate();
          selectedStartDate = generateMetaDateObject(objDate, objDate.getMonth());
          objDate = $scope.getInputEndDate();
          selectedEndDate = generateMetaDateObject(objDate, objDate.getMonth());
      ...
      var snapDateToMonthlyWeekly = function () {
          ...
          var dayDiff = Math.round((selectedEndDate.date.getTime() - selectedStartDate.date.getTime()) / 86400000);
      ...
      $scope.setSelectionRes = function () {
          $scope.selectionDateString = $scope.formatDate($scope.currentSelectedStartDate.date);
          if ($scope.currentSelectedStartDate.date && $scope.isNotSingleDateMode) {
              $scope.selectionDateString += ' - ' + $scope.formatDate($scope.currentSelectedEndDate.date);
          }
      }
      $scope.monthArray = generateMonthArray(null, null);
      ...
      var setStartEndDate = function () {
          var strStartDate = $scope.getInputStartDate();
          var strEndDate = $scope.getInputEndDate();
          if (strStartDate && selectedStartDate) {
              $scope.startDate = selectedStartDate.date.toLocaleString();
          }
          if (strEndDate && selectedEndDate) {
      ...
      $scope.nextMonth = function () {
          ...
          var middleWeek = lastMonth[2];
          var middleDateOfMonth = middleWeek[6];
          var year = middleDateOfMonth.date.getFullYear();
          var month = middleDateOfMonth.date.getMonth();
      ...
      $scope.changeStartDate = function () {
          if (!turnCalendarService.validateDateInput($scope.startDateString, $scope.dateFormat)) {
              return;
          }
          var newDate = turnCalendarService.convertToDate($scope.startDateString);
      ...
      $scope.changeEndDate = function () {
          if (!turnCalendarService.validateDateInput($scope.endDateString, $scope.dateFormat)) {
              return;
          }
          var newDate = turnCalendarService.convertToDate($scope.endDateString);
          setEndDateString(generateMetaDateObject(newDate, newDate.getMonth()));
      };
      $scope.padLeft = function (str, pad, num) {
          if (!/^\d+$/.test(str)) { return str; }
          str += '';
          while (str.length < num) { str = pad + str; }
          return str;
      }
      /**
           * Process a date, and output as the given format.
           */
      $scope.formatDate = function (dateObj) {
          if (dateObj == null || dateObj.length == 0) { return ""; }
          var year = dateObj.getFullYear();
          var month = dateObj.getMonth();
          var date = dateObj.getDate();
          var o = {
              "yyyy": year,
              "yy": $scope.padLeft(year % 100, '0', 2),
              "mmmm": $scope.monthNames[0],
              "mmm": $scope.monthNames[0].substr(0, 3),
              "mm": $scope.padLeft(month + 1, '0', 2),
              "d+": $scope.padLeft(date, '0', 2)
          };
          var strFormat = self.dateFormat.toLowerCase();
          for (var k in o)
              if (new RegExp("(" + k + ")").test(strFormat))
                  strFormat = strFormat.replace(RegExp.$1, o[k]);
          return strFormat;
      };
      $scope.preSelectionString = "";
      /**
           * Listening the open calendar event.
           */
      $rootScope.$on('openturncalendar', function (e, d) {
          if (d != null && d == self.bindingIndex) {
              $scope.enableCalendar();
          }
      });
      angular.forEach([
        'selectionDateString'
          ...
          $scope.$watch(key, function (newVal) {
              if (newVal == $scope.preSelectionString) { return; }
              $scope.preSelectionString = newVal;
              self.dateFormat = $scope.dateFormat = pickValue('dateFormat');
              self.selectionMode = $scope.selectionMode = pickValue('selectionMode');
              $scope.isNotSingleDateMode = self.selectionMode !== 'singleDate';

              var objStartDate = $scope.getInputStartDate();
              objStartDate = resetStartDate(objStartDate);
              selectedStartDate = generateMetaDateObject(objStartDate, objStartDate.getMonth());
              $scope.startDateString = selectedStartDate.date.toLocaleDateString();
              $scope.currentSelectedStartDate = selectedStartDate;

              var objEndDate = $scope.getInputEndDate();
              objEndDate = resetEndDate(objEndDate);
              selectedEndDate = generateMetaDateObject(objEndDate, objEndDate.getMonth());
              $scope.endDateString = selectedEndDate.date.toLocaleDateString();
              $scope.currentSelectedEndDate = selectedEndDate;

              if (!$scope.isNotSingleDateMode) {
                  $scope.monthArray = generateMonthArray(objStartDate.getFullYear(), objStartDate.getMonth());
              } else {
                  $scope.monthArray = generateMonthArray(objEndDate.getFullYear(), objEndDate.getMonth());
              }
      ...
      if (turnCalendarService.validateDateInput(self.startDate, self.dateFormat) && turnCalendarService.validateDateInput(self.endDate, self.dateFormat)) {
          var startDate = resetStartDate(turnCalendarService.convertToDateU(self.startDate, self.dateFormat));
          var endDate = resetEndDate(turnCalendarService.convertToDateU(self.endDate, self.dateFormat));
          selectedStartDate = generateMetaDateObject(startDate, startDate.getMonth());
          selectedEndDate = generateMetaDateObject(endDate, endDate.getMonth());
          $scope.startingYear = self.startingYear = $scope.fullYear = startDate.getFullYear();
          $scope.startingMonth = self.startingMonth = startDate.getMonth();
          $scope.currentSelectedStartDate = selectedStartDate;
          $scope.currentSelectedEndDate = selectedEndDate;
...
.directive('turnCalendar', function () {
    return {
        restrict: 'E',
        require: '^mainCtrl',
        templateUrl: 'App/Views/turnCalendar.html',
        scope: {
            startingMonth: '@',
            startingYear: '@',
            backwardMonths: '@',
