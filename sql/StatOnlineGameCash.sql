...
BEGIN
	declare @level int;
	declare @uid int;
	declare @hisCash float;

	------查看系统表的Sys_Last_Statistic_Date_Online，确认此次统计距离上次统计的时间要大于Sys_statis_Least_period_Online
	...
	if @lastStatDdate is not null and @leastStatPeriod is not null and (GetDate() - @lastStatDdate ) < @leastStatPeriod
	begin	
		set @msg = '此次结算距离上次结算小于最小结算周期！上次结算时间为'; 
		set @msg = @msg + convert(varchar(60), @lastStatDdate);
		return;
	end

	declare @SeasonState bit; 

	exec GetSeasonState @SeasonState output;

	if @SeasonState <> 1
	begin 
		select @msg = '季后赛结束后才能结算，当前系统状态为常规赛！';
		return;
	end
	...
	select class,count(UserID) as userTotal, OGCPTotalCash as cashTotal into #TmpCLass from #TmpUser 
		join OnlineGameCashPayment on ogcpClass = class 
		group by class, OGCPTotalCash order by class;
